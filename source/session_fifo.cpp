...
// SessionFIFO Linking documenataion {{{
/*!
...
{ 
  ...
  if (pid==0) // Orchestrate session initiation
  {
    ...
    { for (int actor2=1; actor2<actors; ++actor2)
      { if (actor==actor2)
        ...
        myInChannels[actor]->Receive(msg);
        myOutChannels[actor2]->Send(msg);
  ...
  else
  {
    ...
    myInChannels.front()->Receive(msg);
    myOutChannels.push_back(new Channel_FIFO(msg.GetData()));
    //cout << "Debug: PID=" << pid << ", received outChannel: " << msg.GetData() << endl;
...
void Session_FIFO::Send(int to, Message &msg) // {{{
{ if (Closed()) throw string("Session_FIFO::Send: Trying to use closed session.");
  if (to<0 || to>=GetActors()) throw string("Session_FIFO::Send: to must be between 0 and actors-1");
  myOutChannels[to]->Send(msg);
} //}}}

void Session_FIFO::Receive(int from, Message &msg) // {{{
{ if (Closed()) throw string("Session_FIFO::Receive: Trying to use closed session.");
  if (from<0 || from>=GetActors()) throw string("Session_FIFO::Receive: to must be between 0 and actors-1");
  myInChannels[from]->Receive(msg);
} //}}}

void Session_FIFO::Delegate(int to, Session &s) // {{{
{ if (Closed()) throw string("Session_FIFO::Delegate: Trying to use closed session.");
  if (to<0 || to>=GetActors()) throw string("Session::Delegate: to must be between 0 and actors-1");
  s.DelegateTo(*myOutChannels[to]);
} //}}}

void Session_FIFO::DelegateTo(Channel &to) // {{{
{ stringstream ss;
  ss << "fifosession://";
...
Session *Session_FIFO::ReceiveSession(int from) // {{{
{ if (Closed()) throw string("Session_FIFO::ReceiveSession: Trying to use closed session.");
...
{ vector<Channel_FIFO*> inChannels;
  ...
  { int pos=address.find(',');
    if (pos<0) throw "Session_FIFO::creator_del: Address bad format.";
  ...
  return new Session_FIFO(inChannels,outChannels,pid,actors);
} // }}}

int _2=Session::RegisterSessionCreator("fifosession",Session_FIFO::creator_del);
